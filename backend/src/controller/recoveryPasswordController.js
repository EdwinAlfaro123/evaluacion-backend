...
recoveryPasswordController.requestCodeMaestro = async ( req, res ) => {
    try {
        const {email} = req.body
        const MaestroFound = await maestroModel.findOne({email})

        if(!MaestroFound){
            return res.status(404).json({message: "NOT FOUND m"})
        }

        const randomCode = cryto.randomBytes(3).toString("hex")
        const token = jsonwebtoken.sign({randomCode, email, userType: "maestro", isVerified: false}, config.JWT.secret, {expiresIn: "15m"})
        res.cookie("recoveryCookie", token, {maxAge: 15 * 60 * 1000})
        const transpoter = nodemailer.createTransport({
            service: "gmail",
            auth: {
...
recoveryPasswordController.verifyCodeMaestro = async ( req, res ) => {
    try {
        const {code} = req.body
        const token = req.cookies.recoveryCookie
        const decoded = jsonwebtoken.verify(token, config.JWT.secret)
...
recoveryPasswordController.newPasswordMaestro = async(req, res) => {
    try {
        const {newPassword, confirmNewPassword} = req.body
        if(newPassword !== confirmNewPassword){
            return res.status(400).json({message: "No coincide"})
        ...
        const passwordHashed = await bcryptjs.hash(newPassword, 10)
        await maestroModel.findOneAndUpdate(
            {email: decoded.email},
            {password: passwordHashed},
            {new: true}
        )
