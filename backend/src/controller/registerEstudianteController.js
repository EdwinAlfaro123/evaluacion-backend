...
registerEstudianteController.register = async ( req, res ) => {
    try {
        ...
        const ExisteEstudiante = await estudianteModel.findOne({email})
        ...
        const token = jsonwebtoken.sign({
            randomCode, name, lastname, email, password: passwordHashed, birthdate, phone, grade, isVerified, timeOut
...
registerEstudianteController.verifyCode = async ( req, res ) => {
    try {
        ...
        const {randomCode: storedCode, name, lastname, email, password, birthdate, phone, grade, isVerified, loginAttemps, timeOut} = decoded

        if(verificationCodeRequest !== storedCode){
            return res.status(400).json({message: "Invalido"})
        }

        const newEstudiante = estudianteModel({name, lastname, email, password, birthdate, phone, grade, isVerified: true})
